...
const App: React.FC = () => {
  ...
  const handleCreateOrder = (newOrderData: Partial<Order>) => {
    ...
    setOrders(prev => {
      // 🚨 중복 주문 방지: 같은 방번호, 같은 아이템, 같은 사용자가 최근 1초 이내에 생성한 주문이 있는지 확인
      // 여러 아이템 동시 생성 시 구분을 위해 시간 범위를 1초로 단축
      const recentDuplicate = prev.find(o => 
        o.roomNo === roomNo && 
        o.itemName === itemName && 
        o.createdBy === currentUser.id &&
        o.status === OrderStatus.REQUESTED &&
        Math.abs(now.getTime() - o.requestedAt.getTime()) < 1000 // 1초 이내 (여러 아이템 동시 생성 시 구분)
      );
